...
- **Turso:** use the HTTP (`@libsql/client` web) client from the Worker — no schema/migration changes; Turso free tier (9GB / 1B reads) unchanged. (Chosen over D1: zero data migration, existing alembic schema intact.)
  - **Vector search — CORRECTED Sept 2026:** Turso's hosted database does **NOT** ship sqlite-vec — `CREATE VIRTUAL TABLE … USING vec0` fails with `no such module: vec0` (verified against a live DB). Phase 1 therefore stores embeddings only as float32 BLOBs in `knowledge_chunks` and ranks them by cosine similarity **inside the Worker** (`src/rag.ts`; fine at single-client scale — a business's FAQ/hours). If a tenant's chunk count ever grows large, the upgrade path is Turso's *native* vector support: `FLOAT32` vector columns + `vector_top_k`, which needs no extension.
...
1. Pick the target model: `gemini-embedding-001`, `outputDimensionality: 768` → **schema-compatible** (float32 BLOB, zero-vector fallback unchanged). If a non-768 model is ever chosen, re-embed all chunks — the dimension is baked into each stored vector.
2. Read all `knowledge_chunks.content` for a tenant; embed in batches with retry/backoff on 429; write vectors as **little-endian float32 blobs** — byte-identical to `sqlite_vec.serialize_float32`. (Turso's hosted DB doesn't ship sqlite-vec, so ranking happens in the Worker — `src/rag.ts`.)
3. `INSERT OR REPLACE INTO knowledge_chunks (id, …, embedding, …)` per chunk — there is no separate vector table, so the chunk `id` primary key makes re-embedding idempotent.
...
| Embedding model swap invalidates vectors | Low | Fresh DB → no migration: pick `gemini-embedding-001 @768` at ingest (§5.3). If a Turso DB already holds nomic vectors: run the idempotent re-embed in §5.6 (LE-float32 blobs, rewrite `knowledge_chunks.embedding` per chunk id), re-verify top-5 hits on sample queries |
| Vector search with no DB extension | Medium | Turso does NOT ship sqlite-vec (verified Sept 2026: `no such module: vec0`); Phase 1 ranks float32 BLOBs in the Worker — fine at single-client scale; upgrade path: Turso native `FLOAT32` columns + `vector_top_k` |
